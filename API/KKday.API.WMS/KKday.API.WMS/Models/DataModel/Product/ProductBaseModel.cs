...
namespace KKday.API.WMS.Models.DataModel.Product
{
    ...
    public class ProductBaseModel
    {
        ...
        public double b2d_price { get; set; }//同業價（套價）....特價的價錢 (計算用) ＝ ref_price (跟Product共用）牌價
        public string display_ref_price { get; set; } //特價的價錢 (顯示用)
        public double sale_price { get; set; } //市價  商品原售價(計算用) (跟Product共用) 
        ...
        public double avg_rating_star { get; set; }//評價星等 (跟Product共用) 
        public Boolean instant_booking { get; set; }//即時預訂 
        public int order_count { get; set; } //訂單數量
        public int days { get; set; }//所需天數  (跟Product共用)
        public int hours { get; set; } //所需時數  (跟Product共用)
        public string introduction { get; set; }//(跟Product共用)
        ...
        public string prod_type { get; set; }//主分類key(跟Product共用)
        public string[] tag { get; set; }//小分類 key(跟Product共用)
