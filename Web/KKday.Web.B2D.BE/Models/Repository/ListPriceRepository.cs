    ...
    public class ListPriceRepository
    {
        ...
        private string GetFieldFiltering(string filter)
        {
            ...
            if (!string.IsNullOrEmpty((string)jObjFilter["name"]))
                _filter += $" AND LOWER(name_first || name_last) LIKE '%{jObjFilter["no"]}%' ";
            // Prod Name
        ...
        //更新黑名單商品
        void UpdateBlacklistProd(B2dBlacklistProduct account, string upd_user)
        {
            BlacklistProdDAL.UpdateBlacklistProd(account, upd_user);
        }
    }
}
