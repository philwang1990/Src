...
namespace KKday.Web.B2D.EC.AppCode
{
    ...
    {
        public static Dictionary<string, string> getuiKey(string locale) //string lang
        {
            Dictionary<string, string> uikey = getKlingon("frontend", locale); //lang
            Dictionary<string, string> uikey2 = getKlingon("system", locale); //lang
        ...
        //挖字專用
        private  static Dictionary<string,string> getKlingon(string webType,string locale)
        {
            try
            {
                //kkredis  
                ...
                if(webType== "frontend")
                {
                    klingon = db.StringGet($"common:uiLangList:{webType}:{locale}");
