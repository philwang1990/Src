...
namespace KKday.PMS.B2S
{
    public class PackageRepository
    {
        ...
        public void Main(PMSSourse pms, long prodOid, long supplierId, string productCode, string currency, Guid supplierUserUuid, string deviceId, string tokenKey, out List<string> timeList)
        {
            timeList = new List<string>();
            try
            {
                _log.Info("PackageRepository start..");
                Console.WriteLine("PackageRepository start..");
                ...
                if (rezdyPackageModel != null && rezdyPackageModel.RequestStatus.Success && rezdyPackageModel.Sessions != null && rezdyPackageModel.Sessions.Any())
                {
                    ...
                    if (packageOid != 0)
                    {
                        ...
                        timeList = availableDate.Select(d => d.ToString("HH:mm")).ToList();
        ...
        public void Update(PMSSourse pms, long prodOid, long supplierId, string productCode, string currency, Guid supplierUserUuid, string deviceId, string tokenKey)
        {
            try
            {
                DateTime startDate = DateTime.Now; //待確認
                ...
                Console.WriteLine("PackageRepository update check..");
                _log.Info("PackageRepository update check..");
            ...
            {
                _log.Error($"Package Update error: {ex}");
                throw ex;
            }
        }
